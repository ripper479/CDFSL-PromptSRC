...
class VLPromptLearner(nn.Module):
    def __init__(self, classnames, clip_model):
        ...
        print("Number of context words (tokens) for Vision prompting: 4")
...
class PromptSRC(nn.Module):
    def __init__(self, n_way, n_support, classnames, change_way=True):
        super().__init__()
        self.n_way = n_way
    ...
    def build_model(self):
        ...
        print("Loading CLIP (backbone: ViT-B/16)")
        clip_model = load_clip_to_cpu()
        clip_model.float()

        print("Building custom CLIP")
        self.model = CustomCLIP(self.classnames, clip_model)
        ...
        self.optim = torch.optim.SGD(self.model.parameters(),lr=0.0025,momentum=0.9,weight_decay=0.0005,dampening=0,nesterov=False)
        # Cosine scheduler
        self.sched = torch.optim.lr_scheduler.CosineAnnealingLR(self.optim, float(50))
        self.sched = ConstantWarmupScheduler(self.optim, self.sched, 1, 1e-05)
